...
from apps.Main.constants import MATH_TYPE, TaskChangeTypes
...
class SavedFilter(models.Model):
    ...
    folder = models.ForeignKey(SavedFilter_Folder, related_name='savedfilters', blank=True, on_delete=models.CASCADE,
...
class TaskNumber(models.Model):
    ...
    chapter = models.ForeignKey(Chapter, on_delete=models.CASCADE, related_name='tasknumbers', blank=True, default=None,
                                null=True)
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
...
class Task(models.Model):
    ...
    # Уcтанавливаем признак изменения задачи
    def save(self, *args, **kwargs):
        op = TaskChangeTypes.newtask if self.id is None else TaskChangeTypes.editask
        super(Task, self).save(*args, **kwargs)
        set_task_changed(self, op, self.author)
...
class Solution(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        set_task_changed(self.task, TaskChangeTypes.solution, self.author)
...
# Таблица изменений в задачах (модерация)
class TaskChanged(models.Model):
    task = models.ForeignKey(Task, on_delete=models.PROTECT)
...
# Записывает признак изменений в задаче при редактировании
def set_task_changed(task, change_type, user):
    if not TaskChanged.objects.filter(task=task).exists():
        tc = TaskChanged(task=task)
    else:
        tc = TaskChanged.objects.filter(task=task)[0]

    if change_type == TaskChangeTypes.newtask:
        tc.newtask = True
    if change_type == TaskChangeTypes.editask:
        tc.editask = True
    if change_type == TaskChangeTypes.mathattr:
        tc.mathattr = True
    if change_type == TaskChangeTypes.solution:
        tc.solution = True
    if change_type == TaskChangeTypes.source:
        tc.source = True
