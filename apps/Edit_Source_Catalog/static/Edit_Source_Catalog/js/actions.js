...
function get_old_parent_dbType(data){
    let ref = $('#source_tree').jstree(true);
...
function get_new_parent_dbType(data){
    let ref = $('#source_tree').jstree(true);
...
function get_parent_dbType(data){
    let ref = $('#source_tree').jstree(true);
    let parent_jstree_id = data['node']['parent'];
    if (parent_jstree_id === '#'){
...
function move_node(data){
    let send_data = {
        'old_parent_dbType': get_old_parent_dbType(data),
		'new_parent_dbType': get_new_parent_dbType(data),
        'old_parent_dbID': get_old_parent_dbID(data),
        'new_parent_dbID': get_new_parent_dbID(data),
        'dbID' : data['node']['a_attr']['dbID'],
        'dbType' : data['node']['a_attr']['dbType'],
...
function delete_node_from_db(data){
    ...
    $.ajax({
        ...
        dataType: 'json',
        success: function(data){
            $('#source_tree').jstree(true).refresh();
...
function create_task_number(){
    let send_data = create_data;
    send_data = JSON.stringify(send_data);
    $.ajax({
        type: 'POST',
        url: 'create_task_number',
        data: {'data': send_data},
        dataType: 'json',
        success: function(data){
            $('#source_tree').jstree(true).refresh();
